...
func recordChannelTestErrorLog(c *gin.Context, channel *model.Channel, err *types.NewAPIError) {
	...
	other.SetPublic("test_error", true)
	// Channel identity is admin-scoped; SetPublic rejects these legacy keys.
	other.SetAdmin("channel_id", channel.Id)
	other.SetAdmin("channel_name", channel.Name)
	other.SetAdmin("channel_type", channel.Type)
