...
// 保留本地错误分类（error_category）派生功能：LogOther 的内部 map 是私有的，
	// 通过 Snapshot 得到扁平视图，分类后再将派生字段写回 public 作用域。
	snapshot := other.Snapshot()
	common.AttachErrorLogCategory(snapshot)
	for _, key := range []string{"error_category", "error_code", "error_type"} {
