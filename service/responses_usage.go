...
// OutputText returns the accumulated generated output text so far. The
// zero-output diagnostic uses it to tell "empty stream" apart from "upstream
// sent no usage for text it did send".
func (a *ResponsesUsageAccumulator) OutputText() string {
	if a == nil {
		return ""
	}
	return a.outputText.String()
}
