...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if summary.PromptTokens > 0 && summary.CompletionTokens == 0 {
		if upstreamStatusCode > 0 {
			other.SetPublic("upstream_status_code", upstreamStatusCode)
		}
		if upstreamChunkSample != "" {
			other.SetPublic("upstream_chunk_sample", upstreamChunkSample)
		}
		...
		if upstreamChunkSample != "" && constant.ErrorLogEnabled {
			errOther := model.NewLogOther()
			if ctx.Request != nil && ctx.Request.URL != nil {
				errOther.SetPublic("request_path", ctx.Request.URL.Path)
			}
			errOther.SetPublic("upstream_status_code", upstreamStatusCode)
			errOther.SetPublic("upstream_chunk_sample", upstreamChunkSample)
			errOther.SetPublic("zero_output", true)
			errOther.SetAdmin("channel_id", relayInfo.ChannelId)
			errOther.SetAdmin("channel_name", ctx.GetString("channel_name"))
			errOther.SetAdmin("channel_type", ctx.GetInt("channel_type"))
			// 记录模型映射信息，与消耗日志展示保持一致
			if relayInfo.IsModelMapped {
				errOther.SetPublic("is_model_mapped", true)
				errOther.SetPublic("upstream_model_name", relayInfo.UpstreamModelName)
			}
			errOther.SetAdmin("use_channel", ctx.GetStringSlice("use_channel"))
