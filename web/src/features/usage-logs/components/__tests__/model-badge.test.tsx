...
describe('mapped model display', () => {
  it('renders the requested model name in a status badge when not mapped', () => {
    const { container } = render(<ModelBadge modelName='gpt-4o' />)
    expect(screen.getByText('gpt-4o')).toBeVisible()
    expect(
      container.querySelectorAll('[data-actual-model="true"]')
    ).toHaveLength(0)
  })

  it('shows the actual mapped model directly below the requested model', () => {
    const { container } = render(
      <ModelBadge modelName='gpt-4o' actualModel='gpt-4o-2024-11-20' />
    )
    expect(screen.getByText('gpt-4o')).toBeVisible()
    expect(screen.getByText('gpt-4o-2024-11-20')).toBeVisible()
    const actualLine = container.querySelector('[data-actual-model="true"]')
    expect(actualLine).not.toBeNull()
    expect(actualLine?.textContent).toBe('gpt-4o-2024-11-20')
  })

  it('renders mapping when requested and actual models are identical', () => {
    const { container } = render(
      <ModelBadge modelName='deepseek-chat' actualModel='deepseek-chat' />
    )
    expect(
      container.querySelector('[data-actual-model="true"]')
    ).not.toBeNull()
  })

  it('keeps the provider icon visible alongside the actual model', () => {
    const { container } = render(
      <ModelBadge
    ...
    // The model name is rendered twice: in the badge and in the mapped line.
    expect(screen.getAllByText('claude-3-5-sonnet-20241022')).toHaveLength(2)
    expect(
      container.querySelector('[data-actual-model="true"]')
    ).not.toBeNull()
    // Claude resolves to a provider icon, which stays visible above the line.
